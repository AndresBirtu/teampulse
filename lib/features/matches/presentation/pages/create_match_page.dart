...
import 'package:teampulse/services/notification_service.dart';
...
class _CreateMatchPageState extends ConsumerState<CreateMatchPage> {
  ...
  Future<void> _prefillTeamName() async {
    try {
      final name = await ref.read(matchRepositoryProvider).loadTeamName(widget.teamId);
      ...
      // Ignored: fallback to manual input if metadata fails.
    }
  }
  ...
  Future<void> _saveMatch() async {
    ...
    try {
      ...
      final creation = await repository.createMatch(
        teamId: widget.teamId,
        teamA: _teamAController.text.trim(),
        teamB: _teamBController.text.trim(),
        date: _matchDate!,
        played: _played,
        goalsTeamA: int.tryParse(_golesAController.text) ?? 0,
        goalsTeamB: int.tryParse(_golesBController.text) ?? 0,
      );

      if (!creation.statsGenerated && context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Partido guardado, pero fallo al generar hoja de stats'),
          ),
        );
      }
      ...
      try {
        final notificationId = creation.matchId.hashCode;
